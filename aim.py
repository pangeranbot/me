...
def NOTIFIED_ACCEPT_GROUP_INVITATION(op):
    #print op
    try:
        sendMessage(op.param1, client.getContact(op.param2).displayName + " WELCOME...!!" + group.name)
    except Exception as e:
        print e
...
def NOTIFIED_KICKOUT_FROM_GROUP(op):
    try:
        sendMessage(op.param1, client.getContact(op.param3).displayName + " Bye...see u next time\n(*´･ω･*)")
    except Exception as e:
        print e
...
def NOTIFIED_LEAVE_GROUP(op):
    try:
        sendMessage(op.param1, client.getContact(op.param2).displayName + " Opsss...!! Good Bye\n(*´･ω･*)")
    except Exception as e:
        print e
    ...
    try:
        ...
        if msg.toType == 2:
            if msg.contentType == 0:
                ...
                elif "Set invite:on" in msg.text:
                    if msg.toType == 2:
                        if msg.from_ in admin:
                            _name = msg.text.replace("Invite:")
                            ...
                            if targets == []:
                                ki.sendText(msg.to,"invitation done")
                if msg.text == "Me":
                    M = Message()
                    M.to = msg.to
                    M.contentType = 13
                ...
                if msg.text == "Jam":
                    sendMessage(msg.to, "Waktu Indonesia Bagian Barat:" + datetime.datetime.today().strftime('%Y年%m月%d日 %H:%M:%S') + " WIB")
                if msg.text == "Gift":
                    sendMessage(msg.to, text="gift sent", contentMetadata=None, contentType=9)
                if msg.text == "Set":
